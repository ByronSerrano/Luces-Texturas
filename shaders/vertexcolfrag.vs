...
void main() {
    ...
    // Inicializar con color base muy tenue
    vec4 result = vec4(0.05, 0.05, 0.05, 1.0);
    
    // Sumar contribución de TODAS las luces
    for(int i = 0; i < NUM_LIGHTS; i++) {
        result += Create_Light(light_data[i].position, light_data[i].color, normal, fragpos, view_dir);
    }

    // Aplicar textura al resultado final
    frag_color = result * texture(tex, UV);
}